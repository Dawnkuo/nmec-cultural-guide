...
const report={scenario:'Upgrade the actual previous single-museum source release to the complete Egypt site',previousRelease:oldManifest.release,release:current.release,testedAt:new Date().toISOString(),offlineRoutes:[]};
try{
 await page.goto(url);assert.match(await page.locator('h1').innerText(),/埃及国家文明博物馆/);await checkpoint(oldManifest.release,'ready');
 ...
 await context.setOffline(true);await page.reload({waitUntil:'networkidle'});assert.match(await page.locator('h1').innerText(),/埃及国家文明博物馆/);report.oldSiteRetained=true;
 ...
 await page.reload({waitUntil:'networkidle'});assert.match(await page.locator('h1').innerText(),/埃及/);assert.doesNotMatch(await page.locator('h1').innerText(),/国家文明博物馆/);
