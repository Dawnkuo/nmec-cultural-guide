...
export function GuideCover({ media, role }: { media: GuideMedia; role: CoverRole }) {
  return <img className={`guide-cover guide-cover--${role}`} src={media.src} alt={media.alt}
...
export function CoverCredit({ media }: { media: GuideMedia }) {
  ...
  return <p className="cover-credit">封面摄影：<a href={credit.sourcePage} target="_blank" rel="noreferrer">{credit.author}</a>
