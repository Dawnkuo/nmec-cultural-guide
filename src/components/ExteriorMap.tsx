...
export function ExteriorMap({exterior,onLocate}:{exterior:ExteriorGuide;onLocate:(nodeId:string)=>void}){
 ...
 return <section className="exterior-map" data-active-exterior={model.id} data-active-exterior-feature={active.id}>
  ...
  <div className="exterior-bottom"><figure><a href={siteUrl(model.referenceImage)} target="_blank" rel="noreferrer"><img src={siteUrl(model.referenceImage)} alt={`${model.title}的来源外观对照`} loading="lazy"/></a><figcaption>{model.referenceCaption} <a href={siteUrl(model.referencePage??`#sources`)}>来源与署名 ↗</a></figcaption></figure><details className="architecture-evidence"><summary>外观来源、精度与边界</summary><p>{model.coordinateSystem}</p><ul>{model.limitations.map(s=><li key={s}>{s}</li>)}</ul>{[...new Set(model.sourceIds)].map(id=>{const s=sourceById.get(id);return s?.url?<a key={id} href={siteUrl(s.url)} target="_blank" rel="noreferrer">{s.publisher}：{s.title} ↗</a>:<p key={id}>来源待核对：{id}</p>;})}<small>{model.parts.length} 个结构构件 · {model.features.length} 个解读观察点 · {model.version}</small></details></div>
 </section>;
}
